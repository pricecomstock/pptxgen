...
// MEDICINE
const disease = flat(loadJsonFromCorporaFile("medicine/diseases.json").diseases);
const infectiousDisease = loadJsonFromCorporaFile("medicine/infectious_diseases.json").diseases;
...
const instrument = loadJsonFromCorporaFile("music/instruments.json").instruments;

// MYTHOLOGY
const greekGod = loadJsonFromCorporaFile("mythology/greek_gods.json").greek_gods;
const greekTitan = loadJsonFromCorporaFile("mythology/greek_titans.json").greek_titans;
const monster = loadJsonFromCorporaFile("mythology/monsters.json").names;

const allNorseGods = loadJsonFromCorporaFile("mythology/norse_gods.json").norse_deities;
...
const plant = loadJsonFromCorporaFile("plants/plants.json").instruments.map(p => p.name);
const flower = loadJsonFromCorporaFile("plants/flowers.json").flowers;

// SCIENCE
const elements = loadJsonFromCorporaFile("science/elements.json").elements.map(e => e.name);
const toxicChemicalJson = loadJsonFromCorporaFile("science/toxic_chemicals.json");
const toxicChemical = toxicChemicalJson.chemicals.concat(toxicChemicalJson.gases);
const weatherCondition = loadJsonFromCorporaFile("science/weather_conditions.json").conditions;
const sizeComparisonFruit = loadJsonFromCorporaFile("science/pregnancy.json").pregnancy.map(p => p.fruit);

// SPORTS
const sport = loadJsonFromCorporaFile("sports/sports.json").sports;
const nflTeam = loadJsonFromCorporaFile("sports/nfl_teams.json").nfl_teams.map(t => t.name);
const nhlTeam = loadJsonFromCorporaFile("sports/nhl_teams.json").nhl_teams.map(t => t.name);
const nbaTeam = loadJsonFromCorporaFile("sports/nba_teams.json").nba_teams.map(t => t.name);
const mlbTeam = loadJsonFromCorporaFile("sports/mlb_teams.json").mlb_teams.map(t => t.name);
const eplTeam = loadJsonFromCorporaFile("sports/football/epl_teams.json").epl_teams.map(t => t.name);
const laligaTeam = loadJsonFromCorporaFile("sports/football/laliga_teams.json").laLiga_teams.map(t => t.name);

// TECHNOLOGY
const programmingLanguage = loadJsonFromCorporaFile("technology/programming_languages.json");
const newTechnology = loadJsonFromCorporaFile("technology/new_technologies.json").technologies;
const knot = loadJsonFromCorporaFile("technology/knots.json").knots;
const computerTechnology = loadJsonFromCorporaFile("technology/computer_sciences.json").computer_sciences;
const appliance = loadJsonFromCorporaFile("technology/appliances.json").appliances;

// WORDS - LITERATURE
const shakespeareSonnet = flat(loadJsonFromCorporaFile("words/literature/shakespeare_sonnets.json").sonnets.map(s => s.lines));
const shakespearePhrase = loadJsonFromCorporaFile("words/literature/shakespeare_phrases.json").phrasess; // sic
const lovecraftWord = loadJsonFromCorporaFile("words/literature/lovecraft_words.json").words;

// WORDS - GENERAL
const emoji = loadJsonFromCorporaFile("words/emoji/emoji.json").emoji;
const kaomoji = loadJsonFromCorporaFile("words/emoji/cute_kaomoji.json").cuteKaomoji;
...
const compoundBreakdown = loadJsonFromCorporaFile("words/compounds.json").compounds // TODO standalone "points"
.map(c => `${c.compoundWord} = ${c.firstWord} + ${c.secondWord}`);
const eggcorn = flat(loadJsonFromCorporaFile("words/eggcorns.json").eggcorns.map(e => e.mistakes));
const encouragingWord = loadJsonFromCorporaFile("words/encouraging_words.json").encouraging_words; // TODO good for picture captions

const ergativeVerb = loadJsonFromCorporaFile("words/ergative_verbs.json").ergative_verbs
const infinitiveVerb = loadJsonFromCorporaFile("words/infinitive_verbs.json")
const resumeVerb = loadJsonFromCorporaFile("words/resume_action_words.json").resume_action_words; // TODO combine with nouns for bullshit resume items
const verb = loadJsonFromCorporaFile("words/verbs.json").verbs.map(v => v.present)

const interjection = loadJsonFromCorporaFile("words/interjections.json").interjections
const noun = loadJsonFromCorporaFile("words/nouns.json").nouns
const personalNoun = loadJsonFromCorporaFile("words/personal_nouns.json").personalNouns

const harvardSentence = loadJsonFromCorporaFile("words/harvard_sentences.json").data
const oprahQuote = loadJsonFromCorporaFile("words/oprah_quotes.json").oprahQuotes
...
const stateOfInebriation = loadJsonFromCorporaFile("words/states_of_drunkenness.json").states_of_drunkenness
